...
** Table of Contents :TOC:
  ...
  - [[#pwa-for-unread-links-to-rss-using-github-actions][PWA for Unread Links to RSS using GitHub actions]]
...
** PWA for Unread Links to RSS using GitHub actions

Create a simple PWA with a share target that accepts URLs with optional tags
and description and stores it locally (if offline) or pushes it to a GitHub
repo, if online. The GitHub repo uses a scheduled action to create an RSS feed
for these articles. The RSS feed should contain the actual contents of the
article extracted using something like [[https://github.com/mozilla/readability][readability]].

The PWA can have a button to clear previous articles from the feed once the
feed has been fetched to a feed reader.

The creation of the RSS feed can be smart about dealing with different types of
URLs - add transcripts for Youtube links, for instance, etc.

This can also be used to create a "share" feed (based on tags) that you can
share with others for sharing stuff that has been already read by you. This
could create a web of shared articles like Google Reader.

The lists to-read and shared will kinda be public. Maybe people with GitHub pro
account can have a private list. Or if they run everything locally in a
container or something. This [[https://eleboog.com/posts/2025-09-07-sharefeed-chrome][chrome extension]] could possibly have some useful
ideas to steal.
